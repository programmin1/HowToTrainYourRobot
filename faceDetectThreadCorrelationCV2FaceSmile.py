...
class webCamGrabber( threading.Thread ):
    def __init__( self ):
        ...
        #self.cam.set(cv.CAP_PROP_FPS, 1)
...
if len( sys.argv[1:] ) == 0:
    ...
    if not os.path.exists(USBSERIAL):
        print('Please change the code, %s not found.' % (USBSERIAL,))
        camThread.stop = True
        exit(1)
    motors = serial.Serial(USBSERIAL, 9600, timeout=5)
            ...
            if camThread.image is not False:
                ...
                if nearFace != None: 
                    ...
                    while result > 10:
                        ...
                        #Move stepper:
                        if fromLeft >.9:
                            motors.write( str.encode('B') )
                            sleep( WAITMOVE )
                        elif fromLeft >.75:
                            motors.write( str.encode('b') )
                            sleep( WAITMOVE )
                        elif fromLeft <.1:
                            motors.write( str.encode('F') )
                            sleep( WAITMOVE )
                        elif fromLeft <.25:
                            motors.write( str.encode('f') )
                            sleep( WAITMOVE )
